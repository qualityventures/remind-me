defmodule RemindMeWeb.ClientNumberControllerTest do
  ...
  describe "create client_number" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, Routes.client_number_path(conn, :create), client_number: @create_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.client_number_path(conn, :create), client_number: @invalid_attrs
      assert html_response(conn, 200) =~ "New Client number"
    end
  end
